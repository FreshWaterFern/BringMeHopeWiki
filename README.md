...
This wiki is made by players, for players — and **you're welcome to pitch in**, no
coding required:
...
See **[CONTRIBUTING.md](CONTRIBUTING.md)** for the full guide (and what *not* to edit
— the item stat boxes are generated automatically and always match the live game).

## 🐛 Found a mistake?

If an item's stats look wrong, or something's out of date, please
[open an issue](https://github.com/FreshWaterFern/BringMeHopeWiki/issues) — that's
...
- `tools/generate_pages.py` — generates item & level pages from the **private**
  game source. Run this when items change.
...
block. Re-running the generator only replaces that block — the community-written
