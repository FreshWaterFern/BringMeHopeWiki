# Heroes Sapphire

<!-- AUTOGEN:START (regenerated from game source; edits inside this block are overwritten on the next run) -->
![Heroes Sapphire](../assets/icons/heroessapphire.png){ .item-icon }

| Property | Value |
|---|---|
| Grade | <span class="grade grade-remarkable">Remarkable</span> |
| Equip slot | Chest |
| Price | 100 gold |
| Max stack | 1 |
| Quest item | No |
| Save id | `heroessapphire` |

**In-game description:** While fighting a boss increase magical resistance by 30%
<!-- AUTOGEN:END -->

## Strategy & Notes

_Community-maintained: add tips, synergies, build ideas, and lore here._
